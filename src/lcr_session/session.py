...
class LcrSession:
    ...
    async def get_user_details(self) -> UserDetails:
        """
    ...
    async def get_json(self, url: str | ChurchUrl, **kwargs: Any) -> Any:
        """
        ...
        * `{uuid}` -- Your unique Church UUID. A few of the API calls use this.
        ...
        if isinstance(url, str):
            return await self._real_get_json(url, **args)
        elif isinstance(url, ChurchUrl):
            return await self._real_get_json(url.render(**args))
        else:
    ...
    async def post_json(
        ...
        * `{uuid}` -- Your unique Church UUID. A few of the API calls use this.
        ...
        resp = await self._client.post(the_url, json=json_data, headers=headers)
        resp.raise_for_status()
        await self._save_cookies()

        return resp.json()

    async def _real_get_json(self, url: str, **kwargs: Any) -> Any:
        """
    ...
    async def _connect(self) -> None:
        ...
        resp = await self._client.get(_AUTH_URLS["login"].render(), timeout=self._timeout)
        resp.raise_for_status()
        ...
        resp = await self._client.get(resp_url)
        resp.raise_for_status()

        # Extract the access token and copy it to a new cookie
        await self._get_token_from_cookies()
        assert self._token is not None, "Token should not be None"
        self._client.cookies.set("owp", self._token)
        self._client.headers.update({"Authorization": f"Bearer {self._token}"})

        # Do the lcr and directory login stuff
        resp = await self._client.get(ChurchUrl("lcr").render())
        resp.raise_for_status()

        resp = await self._client.get(ChurchUrl("directory").render())
        resp.raise_for_status()
